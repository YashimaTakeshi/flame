...
- 6つ目のタブ「刻印」に4本のホイール。**キャプションの設定とは独立**（「ロゴの位置が文字と同期している」
  と指摘された。文字は中央、ロゴは右、のように別々に置けるべき）:
  - **刻印** … なし / 文字 / ロゴ。既定はロゴ。
  - **位置** … 左 / 中 / 右。帯の中の左右。
  - **大きさ** … 小 / 中 / 大。基準サイズの 3 / 4.5 / 6 倍（Medium で一辺 48 / 72 / 96lu）。
...
- **ロゴ** … 富士フイルムの各フィルムの札に倣った**正方形**の配色の版（「長方形になっている、正方形に」）。
  ...
  PROVIA は札が右上、他は左上。PRO Neg. は左に細い色の線、CLASSIC Neg. は下に虹の帯（linearGradient）、
  CLASSIC CHROME は縦縞。文字は Tinos / Arimo / Oswald の Bold（同梱の8ファミリはすべて先読み済み）。
