...
**刻印（`core/badge.ts`、`SceneInput.badge`）** — 最初は写真の右下に置いたが、
...
- 横位置はキャプションの「揃え」に従う。左右の段でも同じ（文字の下に積む）。
- 重ね（余白なし）には帯が無いので置かない。情報タブの列は止め、押すと「余白なしでは刻印を置けません」。
- 見せ方は3択のホイール「刻印」: なし / 文字 / ロゴ。既定はロゴ。
  - **文字** … 地の色に細い枠（ヘアライン）と名前。キャプションと同じ書体、0.85 倍。
  - **ロゴ** … 富士フイルムの各フィルムの札に倣った**配色の版**。緑の札、色の面、名前の文字、下の帯。
    高さは基準サイズの 4.5 倍（Medium で 72lu、幅の約 12%）。参考アプリの札とほぼ同じ大きさ。
    文字は Tinos / Arimo / Oswald の Bold（同梱の8ファミリはすべて先読み済み）。
    **ロゴの絵（ビットマップ）は同梱しない。** 商標の画像を配布物に含める権利がない。配色と文字だけで組む。
    ...
    知らない名前（他社の Vivid など）は地色と文字色を反転した札にする。他社の名前も同じ列で置ける。
- 「載せる項目」の「仕上がり」とは独立。文字列から外しても刻印だけ残せるし、その逆もできる。
- 免責領域は版の外周＋2lu。刻印あり（文字・ロゴ・段の中）の一致は実ブラウザで確認済み。
