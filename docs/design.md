...
### 3.13 入力時の拡大、仕上がり（フィルムシミュレーション等）、刻印（2026-09-22）

実機の4巡目と5巡目。「情報を入力しようとすると画面が拡大されて、どこを入力しているか分からなくなる」、
「ピクチャーコントロールやフィルムシミュレーションを情報に入れたい」、
「ロゴを置きたい。ただし**写真の中には入れない**。フレームの中に。文字とロゴを選べるように。
名称は富士フイルムの呼び方でなく各社に通じるものに」。
...
**仕上がり（`FieldId 'film'`）** — 8つ目の項目。画面での名前は「仕上がり」。
フィルムシミュレーション（富士）・ピクチャーコントロール（ニコン）・ピクチャースタイル（キヤノン）・
クリエイティブルック（ソニー）・フォトスタイル（パナソニック）は、どれも1社の呼び名なので、
共通語の「仕上がり」を使う。内部の識別子 `film` はそのまま（変える理由が無い）。
行の型ではレンズの直後に置く（落とす順は末尾からなので、露出・焦点距離より後まで残る）。値の出どころは2つ:
...
- **手入力**。FUJIFILM 以外は MakerNote の形式が各社で違い、対応表も公開されていないものが多いので、
  自動では読まない。情報シートの「仕上がり」欄に入れる。`<datalist>` に富士の名前と各社共通の名前
  （Standard / Vivid / Neutral / Portrait / Landscape / Flat / Monochrome）を候補として出す。

**刻印（`core/badge.ts`、`SceneInput.badge`、「刻印」タブ）** — 最初は写真の右下に置いたが、
「写真の中には入れないでフレームの中に」と直された。いまは**キャプションと同じ帯の中、文字の下**に積む。
- 帯の高さは compose が「文字の高さ＋隙間＋刻印の高さ」で決めて `resolveLayout` に渡す。
  刻印は帯の中の塊なので、写真と重なることは構造上ない（総当たりテストで固定）。
  元比（OR）では刻印のぶんキャンバスが伸び、固定比では写真が縮む。
- 重ね（余白なし）には帯が無いので置かない。列は止め、押すと「余白なしでは刻印を置けません」。
- 6つ目のタブ「刻印」に6本のホイール。**キャプションの設定とは独立**。
  「文字と同じ数だけ、上下左右を選べるように」と言われ、文字の「文字（辺）＋寄せ」に横を足した:
  - **刻印** … なし / 文字 / ロゴ。既定はロゴ。
  - **辺** … 上 / 下 / 左 / 右。どの辺の帯に置くか。キャプションと同じ辺なら同じ帯を分け合う。
    別の辺なら `resolveLayout` がその辺に**もう1本の帯**を取る（`ExtraBand`）。
    上下の帯は高さ＝刻印の高さ、左右の帯は幅＝刻印の幅。写真の箱はその分だけ縮む（元比では伸びる）。
  - **横** … 左 / 中 / 右。帯の中の左右。
  - **縦** … 上 / 中 / 下。帯の中の上下。
  - **大** … 小 / 中 / 大。基準サイズの 4 / 6 / 8 倍（Medium で一辺 64 / 96 / 128lu）。
  - **枠** … なし / あり。地の色でヘアラインを外周に回す。黒地に ETERNA、白地に Velvia のように
    地色と版の色が同じで輪郭が消えるときのため。文字のときも同じ枠。
- **同じ帯を分け合うときの規則**（compose.ts）。帯に要る高さは、文字と刻印が**横に重なる**なら
  「文字＋隙間＋刻印」、重ならない（文字は左、刻印は右）なら高い方。置くときは文字を寄せ（captionAlign）、
  刻印を自分の横・縦で置き、それでも重なったら文字を先に刻印をその下に積んでまとめて寄せる。
  総当たり（寄せ3 × 揃え3 × 横3 × 縦3 × 大きさ3）で重ならないことを固定。
- **左右の帯の縦の範囲**。上下にも帯があるときは写真そのものの高さに限る。写真が箱いっぱいでないとき
  上下の帯は写真の端から始まるので、左右の帯を箱の高さで取ると写真の下で交差する
  （実測: 16:9・写真上・文字下に左の刻印で3行目と重なった）。上下に帯が無ければ従来どおり箱の高さ。
  この一般化で、キャプションだけのときの結果は従来と同じ（全 2,610 通り × 5 比 × 3 高さで突き合わせ。
  唯一の差は元比・左右の段で、写真の箱の幅の取り方が揃って高さが 1〜2lu 縮んだ）。
- **文字** … 地の色で名前。0.7 / 0.85 / 1.05 倍。キャプションと同じ書体。
- **ロゴ** … 富士フイルムの各フィルムの札に倣った**正方形**の配色の版（「長方形になっている、正方形に」）。
  設計は 100×100 で、緑の札・色の面・名前・下の帯を矩形と文字だけで組む。
  「デザインが全然違う」と言われて参考の札を1つずつ写し直した: PROVIA は右上に大きな緑と青い面と下の黒、
  Velvia は白い面に左の緑線と下の緑、PRO Neg. は左に細い色の線と PRO / Neg. の二段、NOSTALGIC Neg. は
  縦長の1行と赤・橙・黄の帯、CLASSIC Neg. は黄土色に虹の帯と右下の緑、CLASSIC CHROME は焦げ茶に橙と赤の縦縞、
  ASTIA はカーキに下の紺、ACROS は上が明るい灰で下が炭色にサンセリフ、ETERNA BLEACH BYPASS は上下二段。
  文字は箱いっぱいに入れる（高さ 1.0、幅 0.96）。控えめに取ると別物に見えた。
  書体は Tinos / Arimo / Oswald の Bold（同梱の8ファミリはすべて先読み済み）。
  **ロゴの絵（ビットマップ）は同梱しない。** 商標の画像を配布物に含める権利がない。配色と文字だけで組む。
  版の文字は箱の幅を実測で越えないよう縮めるので、想定と違う書体でも溢れない。
  読み取れる名前（PROVIA … SEPIA、BLEACH BYPASS）はすべて専用の配色を持つ（テストで突き合わせる）。
  知らない名前（他社の Vivid など）は地色と文字色を反転した札にする。
- 「載せる項目」の「仕上がり」とは独立。文字列から外しても刻印だけ残せるし、その逆もできる。
- 免責領域は版の外周＋2lu。刻印あり（文字・ロゴ・段の中・右寄せ・大・枠）の一致は実ブラウザで確認済み。

**左右の段の中での揃え（2026-09-22・6巡目）** — 縦位置の写真を左に置き文字を右にすると、右の余白が広いのに
文字が左端に貼り付き「余白の中央に置けない」と指摘された。原因は2つ:
(1) 段の行構成が `alignOverride: 'left'` で左揃えに固定されていた（参考アプリの段のスタイルに倣ったもの）。
(2) 段の置き場所が固定幅 300lu の帯で、余りの広さと無関係だった。
直し: 固定をやめて揃えは利用者の選択に従う。文字は従来どおり 300lu の幅で**組む**が、**置く**のは
写真の端＋隙間からキャンバスの端−余白までの余り全部（`band`）で、その中で左・中・右に揃える。
刻印の帯（別の辺に取るもの）は従来どおり端に刻印の幅だけ取る。
同じ段を分け合う刻印との重なりは、組んだ幅ではなく実際の帯の幅で見直す。
テストで「中央揃えは余りの真ん中、右揃えは右の余白の内側の端、左揃えは写真の右端＋隙間」を固定。

**ヘッダーのかすれ（2026-09-22・7巡目）** — 「上部のアイコンや文字にグラデーションが掛かっている」
と指摘された。調べると**掛かっていなかった**。実機の画面を画素で測ると、名前は副次色（--ink-2 = 168）の
細字で最大 114、印は線の太さ 1.6 で最大 218 / 205 しか出ておらず、本来の 168 / 242 に届かない。
高解像度で細線が薄く溶け、行全体がかすんで「グラデーション」に見えていた。
直し: 名前を地の色（--ink）の 600 に、印を 22px・線の太さ 1.8 に上げ、ヘッダーに地色を明示した。
測り直して3つとも 239（目標 242）。**見え方の訴えは、まず画素で測ってから直す。**

---

### 3.14 設定の保存・アプリの共有・アプリの印（2026-09-22）

**設定の保存（`app/state/persist.ts`）** — 次に開いたときも同じ好みで始まるようにした。
- **保存するのは「好み」だけ。** 配置6軸・書体・地色・揃え・字間・大きさ・枠・載せる項目・刻印の6つ、
  それに作者名。**保存しないのは「その1枚のこと」**: タイトル・切り取りの位置・カメラ名などの手入力。
  持ち越すと写真を変えるたびに消して回ることになる。
- **読み戻しは1項目ずつ検証する。** 版を上げて選択肢が消えても、壊れた1項目のために全部を失わない
  （その項目だけ既定に戻る）。配置6軸は読んだあと必ず `normalize()` に通す。
  載せる項目は「保存に無い項目は既定のまま」にするので、項目が増えても消えない。
- 見張るのは store 全体で1か所。軸を足したときに「ここにも保存を書き忘れた」が起きない。
- ★書き込みのまとめ方で1度壊した★: 同じ中身で呼ばれるたびに待ち時間を延ばしていたため、
  全面で写真を指で動かしている間は一度も保存されなかった（store 全体を見張っているので
  focus が動くたびに呼ばれる）。「いま待っている中身」と同じ呼び出しは何もしないようにした。
- 画面が隠れるとき（`pagehide` / `visibilitychange`）に待っている書き込みを済ませる。
  iOS は裏に回った頁を黙って終わらせるので、400ms の待ちが返ってこないことがある。
- 「初期値に戻す」は**工場出荷**へ戻る（保存された好みへではない）。戻した結果がそのまま保存される。

**アプリの共有（`app/share.ts`）** — 友人に渡しやすくした。渡すのは URL ひとつ。
共有シート（`navigator.share`）→ クリップボード → URL をそのまま見せる、の順に落ちる。
★やめたときを失敗として扱わない★: 共有シートを閉じると `AbortError` が飛ぶ。これを失敗にすると
自分でやめた人に落ち度があるように見えるし、勝手にコピーへ落ちると驚かせる。
配る URL は検索語や現在地を落とした入口だけ。置き場所はホーム画面（新しい人が最初に見る場所）と
書き出しシート（作ったものを見せた直後が、人に教えたくなるとき）の2か所。

### 3.15 仕上がりの札を、描き起こしから支給された画像に変えた（2026-09-22）

刻印のロゴを矩形と文字で描き起こしていたが、実機で「デザインが全然違う」と2度言われ、
**原画（フィルムの箱の絵）が支給された**。13枚（PROVIA … SEPIA）。

**core は絵を持たない。** `BadgeSpec.image` に載るのは識別子と縦横比だけで、
実体は実行層が解決する（写真とまったく同じ扱い。`photo` op を出す）。
この形にしたので、解像度不変の性質は写真と同じ理屈でそのまま成り立つ。

- 寸法は**高さが大きさの指定で決まり、幅は札の比に従う**。段に入らなければ幅で止め、比は崩さない。
  比は画像が届く前に要る（帯の大きさを先に決める）ので、目録に w/h を持たせて比だけ先に渡す。
- 名前からファイル名は引かない。`public/film/manifest.json` が正（`scripts/build-film.py` が書く）。
  EXIF から読む名前（`CLASSIC Neg.` のように空白と点を含む）をそのまま鍵にし、
  URL に出す名前だけ英数字に直す。同梱アセットの検査が実体・大きさ・sha256 を突き合わせる。
- 読み込みは名前が決まってから1枚だけ。届くまでは「名前だけの札」で、届いたら組み直す。
  ★画面は籠の中を覗かない★: `ensureFilmLogo` が札そのものを返し、画面はそれを状態に持つ。
  覗く形（モジュールの籠を描画中に読む）だと、籠が変わっても React が気づかない。
  読み込んだ札は名前と対にして持つ。そうしないと仕上がりを切り替えた直後の一瞬だけ
  前の札が新しい名前で出る。
- **描き起こしの版は捨てた。** 似せきれない絵を置くより、札を持たない名前
  （`MONOCHROME` / `BLEACH BYPASS` や他社の「ビビッド」）は
  地色と文字色を反転した**名前だけの札**に落とすほうが正直で、他社にも同じ形で使える。
- 原画は持ち主から支給されたもので、富士フイルム株式会社の製品名・商標を含む
  （`assets/film/README.md`）。作り直したものではない。

---

### 3.16 PC の組み方と、保存先を選ぶ保存（2026-09-23）

「PC で開くとスマホの画面をそのまま横に伸ばしただけで使いにくい。設定を右に寄せて
写真を大きく見ながら編集したい。選び方も PC 向けに」「保存はエクスプローラー／Finder で場所を選びたい」。

**組み方を2つ持つ（`app/layout.ts`）** — 幅 960px 以上を desk、それ未満を phone。分けるのは幅だけ。
- phone … 上から ヘッダー / プレビュー / オプション行 / タブバー。1タブずつ、ホイールで選ぶ（従来）。
- desk … 左にプレビュー、右に 380px の設定の欄。**6つの節が全部見えていて**、タブは無い。
  節の見出しはタブと同じ印と名前にして、スマホで覚えた場所がそのまま通じるようにした。

**選び方を替える（`ui/Choice.tsx`）** — 各パネルは `Choice` だけを使い、どちらの見た目かを知らない。
phone ではホイール、desk では押しボタンの並び（`ui/Segmented.tsx`）。ホイールは指で回すための形で、
鼠では「回す」が「小さく引っ張る」になって狙いにくい。PC では選択肢を全部見せて1回押す。
- 押せない値は薄く残し、押されたら理由を知らせる（ホイールと同じ規則）。
- 書体と地色は選択肢が多いので 2 列の升に折り返す（名前は書体自身で、色は丸と名前で）。
- 鍵盤は矢印で隣へ。選んでいる1つだけが Tab で止まる（radiogroup の慣例）。
- 見た目を足すときに直す場所は `Choice` の1つだけ。

**面（Sheet）は PC では真ん中の窓** — 下から出る面は指の作法。PC では幅 560px の窓を中央に出す。

**保存先を選ぶ（`platform/save.ts`）** — File System Access API（`showSaveFilePicker`）。
Chromium 系の PC にしかないので、あるときだけ第一候補にする。
★実機で分かったこと★: PC の Chromium も `navigator.share` にファイルを渡せるので、
スマホと同じ「共有が先」のままだと **Windows の共有パネルが開く**。そこで `saveImage` に
「どれを先に試すか」（auto / picker / share / download）を持たせ、画面が組み方で選ぶ:
- desk … 「名前を付けて保存…」（窓が無ければ「ダウンロード」）を第一に、
  「既定の場所にダウンロード」と「共有…」を脇に。
- phone … 従来どおり共有シート（写真アプリに入る）が第一。
窓を閉じただけなら取りやめとして扱い、勝手に共有やダウンロードへ落ちない。
窓を開けない環境（権限なし・枠の中）ではダウンロードへ落ちる。
窓を開く権利は利用者の操作に紐づくので、押した手続きの中で **await を挟まずに** 呼ぶ（共有と同じ）。

**検査（`scripts/shot.mjs`）** — 幅 1440 でも開き直し、6 節・押しボタンの並び・ホイール無し・
タブバー無し・欄からのはみ出し無し・比率を押すと正方形になる・書き出しの窓が中央、を数える。

---

**アプリの印（`scripts/build-icons.py`）** — 1枚の原画から5つの印を作る。
原画は透過の市松模様が焼き込まれた書き出しなので、角丸の外側を切り抜いて透過に戻す。
★切り抜きで絵を削っていないこと★を、切った画素がすべて市松（無彩色で明るい）だったかを数えて確かめる。
1画素でも絵を削っていたら止まる（半径 0.205 はこの検査で決めた）。
写真を含む絵なので、そのままだと 512px が 430KB になる。色と透過を分けて扱い、色は誤差拡散つきで
255 色に落とし、残る1色を透過の席にする（透過がある版は八分木でしか減色できず、空に段が出るため）。
結果 129KB。iOS のホーム画面用は透過が黒く落ちるので不透過の版を、Android の適応アイコン用は
中央 80% に収めた不透過の版を別に作る。同梱アセットの検査に大きさの上限を足した。

**仕上がりの入力は選択式** — 最初は `<datalist>` にしたが、iOS Safari では文字を打つまで候補が出ず、
「テキストでしか入力できない」と見えた。`<select>` にして OS のピッカーで選ばせる。
先頭は「写真の値」（読めていればその名前）、続けて富士の名前と各社共通の名前、末尾に「その他（手で入れる）」。
その他を選んだときだけ下に入力欄が開く。
...
### 3.17 窓の縮小で消えるプレビュー、重ねでも出る刻印、スクロール不要の書き出し窓（2026-09-23）

「全画面から窓を小さくすると画像が消える」「重ねのときも仕上がりのロゴを出したい（PC・スマホとも）」
「書き出しの窓でスクロールしないとボタンに届かないのが面倒。画像の大きさを調整して収めて」。

**縮小で消える（`usePreview.ts` / `usePan.ts`）** — desk と phone で canvas の親が違うので、
組み方が切り替わると React が canvas を**作り直す**。ref は同じ変数のまま中身だけ差し替わるため、
ref にしか依存していない描画と指の効果は再実行されず、新しい canvas は一度も描かれずに真っ暗だった。
組み方（`layout`）を「作り直しの合図」として両方の効果に渡す。`shot.mjs` に 1440→480→1440 と窓を
変えて **画素で描かれていること**（色数 > 8）を見る検査を足した（実測: 縮小後 241 色、戻しても描画あり）。

**重ねでも刻印（`core/compose.ts`）** — 従来は重ね（`caption: 'overlay'`）で刻印を出さなかった。
帯が無いので「辺」は選べないが、横・縦・大きさ・枠はそのまま効く。置き場は写真の上、
外周の余白（`outerInsetLu`）だけ内側に入れた矩形の中。文字と重なるときだけ文字の上に逃がす
（文字の行の横幅と実際に交わるときだけ。交わらなければそのまま）。墨は重ねの墨（写真の上の白系）。
刻印の欄では辺だけを薄くして「余白なしでは写真の上に置きます。辺は選べません」。
並行性の検査（`parity.browser.ts`）に「刻印 重ね」を足し、単体は「重ねでは刻まない」を「重ねでは写真の上」に置き換えた。

**書き出しの窓（`ExportSheet.tsx` / `Sheet.tsx`）** — 画像を幅いっぱいにしていたので、縦長の画像では
ボタンが窓の下に隠れた。窓の本体を「残りの高さ（画像）／ボタン」の2段の格子にして、
画像は `max-width/max-height: 100%` で残りの高さに収める。
★実測で分かったこと★: PC では窓の高さが `max-height` だけ（不定）だったため「残りの高さ」が定まらず、
画像が窓からはみ出してボタンに被った。tall の面は PC でも高さを**決める**（`min(88vh, 900px)`）。
★さらに実測で分かったこと★: それでも **縦長（9:16）だけが下を切られていた**。
`max-height: 100%` は親の高さが（格子の `1fr` のように）その場で確定しないと効かず、
9:16 の画像が 935px に伸びて 505px の枠からはみ出していた（横長と正方形は幅で決まるのでたまたま収まっていた）。
画像を枠に**絶対位置で貼る**（`inset: 0` ＋ `width/height: 100%` ＋ `object-fit: contain`）。
絶対位置の子は親の実寸に対して割合が解けるので、確実に収まり、余る側は contain が空ける（切り取らない）。
影は箱ではなく写った絵の形に落とす（`filter: drop-shadow`。contain で余る側に影が出ないように）。
`shot.mjs` は phone（9:16 と 16:9）と desk（9:16）で、画像が本体に収まり・枠からはみ出さず（`spill: 0`）・
本体がスクロールせず・ボタンが全部見えていることを数える。**縦長で確かめる**のが要点。

**公開し直した直後の真っ黒な画面（`index.html`）** — 実機で「なにもひらかない」。
題は出るのに中身が無く、背景だけが黒。原因はアプリではなく**公開のしかた**だった。
公開し直すと部品（JS・CSS）の名前（指紋）が変わり、古い名前のファイルは消える。
GitHub Pages は HTML を約10分持たせるので、その間に開くとブラウザは**古い index.html** を出し、
そこが指す部品は 404 になる。`color-scheme: dark` があるため、CSS が無い素の画面は真っ黒に見える。
（10分後に開いたら直った、という実機の報告が裏づけになった。）
対処は、部品の取りこぼしに気づいたら**一度だけ**取り直すこと。`index.html` の先頭に素の JS を置き、
script／link の失敗を捕まえたら（念のため読み込み後 2.5 秒経っても何も描かれていなければ）
`?v=<時刻>` を付けて開き直す。キャッシュを外すためで、印は描けたら URL から消す。
2度は繰り返さない（本当に壊れているときに往復し続けないよう、印を sessionStorage に置く）。
`shot.mjs` に、消えた名前を指す古い HTML を返す置き場を立てて、**自力で立ち直ること**を見る検査を足した。

### 3.18 「黙って効く」6つ（2026-09-23）

機能案を5つの視点で出して採点したうち（採点つき 26 案）、上位の「UI を増やさずに全員に効く」6つをまとめて入れた。
いずれも新しいボタンは無い。

**撮影情報を書き出しに書き戻す（`platform/exif-write.ts`）** — canvas から出た JPEG には EXIF が無く、
写真アプリや Lightroom で「今日撮った写真」に並んで旅行の順番が崩れる。元の写真の APP1（Exif）だけを
自前で切り出して piexifjs で辞書にし、**日時・カメラ・レンズ・露出だけ**を戻す（§11.8）。
GPS と MakerNote とサムネイルは戻さない。Orientation は 1、画素数は書き出した大きさ、Software はこのアプリ。
APP1 は APP0（JFIF）の直後に差し込み、元の JPEG は読み直さず `Blob` の断片で組む（8MB を文字列にしない）。
戻したあと exifr で読み直して Model が一致することを確かめる。**失敗しても写真は返す**（おまけは本体を落とさない）。
元が HEIC などでセグメントが無いときは、読み取り済みの値から最小限の辞書を組む。
ファイル名も撮影日時から付ける（`fuchidori-<撮影日>-<撮影時刻>.jpg`。§16.1）。
検査: `shot.mjs` が PC の書き出し窓の data: URL の先頭を読み、Exif・機種名・撮影日・Software があり GPS が無いことを見る。

**タイトル未入力なら焼かない（`state/doc.ts`）** — 既定の 'Untitled' を毎回フチに焼いていたのをやめ、既定を空にした。
空なら行そのものが詰まる（§4.2 の欠損の詰め方）。配られた人の最初の1枚が「Untitled って何？」にならない。

**LINE の中のブラウザから抜け出す（`share.ts` / `platform/save.ts`）** — 配る URL に LINE の約束事
`?openExternalBrowser=1` を付け、LINE で受け取っても Safari / Chrome で開くようにした。
それでもアプリの中のブラウザで開いたとき（UA に Line/・Instagram・FBAN）は、書き出し窓に
「右上の「…」から「ブラウザで開く」」を1行だけ添える。判定は UA の印だけで、外れても案内が1行増えるだけ。

**日付は壁時計（`core/wallclock.ts`）** — exifr が返す `Date` を使うと、EXIF の日時（タイムゾーンを持たない）が
端末のタイムゾーンで読み替えられ、**海外で撮った写真の日付が1日ずれていた**（§4.4 違反）。
`reviveValues: false` で生の文字列を受け、`WallClock {y,m,d,hh,mm,ss}` として数字のまま持ち回る。
手入力の日付も、ファイルの更新日時も同じ型。あわせて日付の書き方（2026.09.20 / 2026.9.20 / 2026/09/20 /
2026年9月20日 / 2026-09-20 / 時刻つき）を情報シートの1つの選択肢で選べるようにし、好みとして保存する。
選択肢の表示は見本そのもの（「dots」のような名前で選ばせない）。

**戻る操作で写真を失わない（`state/ui.ts`）** — 面（情報・書き出し・自己診断）を URL の印（#info など）と対にして
履歴に載せた。iPhone の戻るスワイプやブラウザの戻るで、アプリごと離れるのではなく面が1段閉じる。
自分で履歴を積んだときだけ `history.back()` で閉じ、印つきの URL で開かれたとき（しおり・手打ち）は
印だけ消す（戻ると外へ出てしまうため）。写真が無いのに #info / #export で開かれたら面は出さない。
写真を読み込んだあとの再読み込み・タブ閉じは `beforeunload` で確認する（iOS Safari は出さないが PC で効く）。
検査: `shot.mjs` で面を開いて `history.back()` → 面が閉じ、写真が残り、印が消えること。✕ で閉じても印が残らないこと。

**PC の入口（`App.tsx`）** — 窓に落とす・Ctrl/⌘+V で貼る。落とせるときは縁を光らせる。
画像でないファイルを落としても既定の動き（ブラウザがそのファイルを開く）は止める。
...
- 通る経路は Chromium の版しだい。手元の Chromium は H.264 を書けず WebM、**CI の Chromium は書けるので MP4（H.264）** を通る。
  CI では MP4 の映像までが自動で確かめられる。音声は、試験用の動画が Opus で CI の Chromium は AAC を作れないため
  「映像だけ」になる（落として告げる、の経路が確かめられる）。iPhone の動画は AAC なので写せる。**iPhone の実機で確かめる。**

### 3.21 動画の音と、編集中の再生（2026-09-23）

実機で「動画を確かめたが音が出ない」「書き出すまで動画のプレビューを見られるようにしてほしい」。

**音が出なかった原因（3つとも直した）**
1. **写せる AAC まで作り直していた。** mediabunny は音声に品質（quality）を指定すると必ず作り直す
   （`conversion.ts` の `needsTranscode = … || !!trackOptions.quality`）。iPhone の AAC も作り直しになり、
   AAC を作れない端末（iPhone の Safari など）では音声ごと落ちた。**品質を指定しない。**
   そのまま写せるときは何も指定せず、作り直すときも既定の品質（高）で作られる。
2. **読めない音声を選びうる。** iPhone 16 の空間オーディオは、読めない形式（APAC）の音声を AAC と別に持つ。
   「主な音声」（ビットレートの高い方）に任せると読めない方を選び、音声が落ちる。
   **音声を全部並べ、写せる／読める方を自分で選ぶ**（`planExport`。映像も主な1本以外は捨てる）。
3. **書き出し結果の再生欄を消音で自動再生していた。** 鳴らないのは当然で、「音が出ない」と受け取られた。
   **消音にしない・勝手に流さない。** ▶ で音付きで流れる。押すまでは書き出した最初のコマ（表紙）を見せる。

**入れ物と音声の決め方（`planExport`。純粋な関数で、単体で全部の組み合わせを確かめる）**
1. MP4 に AAC をそのまま写す（iPhone・多くのカメラ）
2. MP4 に AAC を作り直して入れる（読める音声があり、AAC を作れる端末。富士の非圧縮音声を PC で）
3. MOV に非圧縮音声をそのまま写す（富士の MOV を、AAC を作れない端末で。Apple の写真アプリで鳴る）
4. WebM に Opus（MP4 では音を残せないが WebM なら残せる環境。Linux の Chrome など）
5. MP4 で映像だけ（落として告げる）

**編集中の再生（`useVideoPlayback.ts`）** — 画面に置かない `<video>` で繰り返し再生し、新しいコマが出るたびに
プレビューを描き直す（`requestVideoFrameCallback`。無ければ毎フレーム時刻を見る）。描き直しはプレビューの
いつもの経路で、写真の代わりにこの `<video>` を渡すだけなので、枠・文字・刻印の位置は書き出しと同じ。
`usePreview` には「描き直しの合図を受け取る口」（subscribe）を足した（rAF で合流するので1フレームに1回）。
- 音は消して始まる（ブラウザは音付きの自動再生を許さない）。プレビューの上の印（スピーカー＋長さ）で出し入れする。
  印はプレビューの上に段を空けて置く（枠や写真に重ねない）
- 書き出しの窓を開いている間・画面が隠れている間は止める。窓を閉じると続きから流れる
  （操作の外で音付きの再生を拒まれたら、消音にして流し直し、印で知らせる）
- 自動再生を止められたら（iPhone の低電力モードなど）▶ の印。押せば流れる
- この端末の `<video>` で流せない形式（Windows の一部の HEVC など）、向きの読み方が書き出し側と食い違うとき
  （縦横比が 2% 以上ずれる）は、最初のコマのまま（静止）
- 描き手は `width` / `height` から切り出し範囲を計算する。`<video>` では属性の値なので、実寸を入れておく

**書き出しの最中の映像** — 描き上がったコマを間引いて（120ms ごと・長辺 960）小さなキャンバスに写す。
待っている間も仕上がりが見えている（空の画面と線だけにしない）。進み具合の線は映像の足元に置く。

**検査**
- 単体: `planExport` 8 件（iPhone の AAC をそのまま／空間オーディオを避ける／読めない音声だけなら映像だけ／
  富士の非圧縮音声は AAC か MOV／WebM への逃げ道／書けない端末）
- 実ブラウザ: 音声が2本（AAC と Opus）入った試験用の動画をその場で作り、写せる方を選んで音が残ること。
  AAC は作れないので、無音の AAC-LC のコマ（hls.js が使うもの）を直接並べる（写すだけなら中身を読まない）。
  手元の Chromium は AAC を読めないので Opus を選び（WebM）、CI の Chromium は H.264 を書けるので
  **AAC をそのまま写す（MP4）＝ iPhone と同じ経路**を通る
- 画面（`shot.mjs`、公開時の CSP の下で）: 編集中にコマが進む／音の印が切り替わる／書き出しの最中に映像が流れ、
  編集のプレビューは止まる／結果は消音でも自動再生でもなく、表紙がある

### 3.22 UI/UX 見直し・第0段：どの設計でも残る直し（2026-09-23）

利用者の目で洗い出した29件（U01〜U29）のうち、画面の作り直し（第1段）に左右されないものを先に入れた。

- **取り消しは何段でも、やり直しもできる**（`state/doc.ts`）。最大 50 段。続けざまの同じ種類の変更
  （ホイールを回す等）は 600ms 以内なら1段にまとめる。見出しの真ん中に ↶ ↷（写真を開いた後は題の代わり）。
  PC は Ctrl/⌘+Z・Ctrl/⌘+Shift+Z（Ctrl+Y）、Ctrl/⌘+S で書き出し。文字の欄の中では横取りしない
- **情報タブの ↻ は情報だけ戻す**（載せる項目・日付の書き方・タイトル・手入力）。以前は配置も書体も工場出荷に戻り、
  取り消せなかった。戻したら「情報を戻しました（↶ で元に戻せます）」
- **写真を替えたら**タイトル・手入力・切り取り・履歴を捨てる（前の写真のタイトルが次の写真に載っていた）
- **撮影情報が無い写真で「入れない」**はその1枚だけ（`skipShotFacts`）。保存される「載せる項目」は書き換えない
- **キャプションの表記を整える**（表示だけ。書き戻す EXIF は元のまま）: iPhone の「Apple 」、
  レンズ名の「iPhone 16 Pro back camera 6.765mm f/1.78」を落とす（焦点距離・露出は別の項目で出る）。
  他社レンズは「 | Contemporary 021」の系列名だけ落とす。F 値は製品名の一部（XF27mmF2.8）なので残す
- **刻印タブ**: 仕上がりの記録が無い写真では6列とも止め、押すと「仕上がりの記録がない写真です（✎で選べます）」
- **情報シート**: ✓・背後のタップ・戻るスワイプは反映して閉じる。✕・Esc だけがやめる。何欄変えても取り消しは1段
- **読み込み中・書き出し中が見える**: ホームの枠が息をして文を出す／差し替え中はプレビューを薄める／
  写真の書き出し中は今のプレビューを薄く置き、行き来する線を出す
- **比率の並び**: 元比 → 4:5 → 3:4 → 1:1 → 9:16 → 16:9
- タブの選択の見せ方は見比べ用の切り替え（`?tabs=b|c`）で比べ、第1段の「白い字＋上の細線」に決まった（§3.23）

### 3.23 UI/UX 見直し・第1段：設定の整理と操作の部品（2026-09-23）

依頼者の指摘:「文字重ねと余白なしが同じ」「余白なしだと文字を上に置けない」「全部ボタン式で分かりづらい。
Lightroom のように洗練された形に」。モックで案を見せ、承認を得て入れた（余白なしでも上下左右に置ける、は依頼者の追加）。

**設定の整理（型から直した）**
- `CaptionPlace` から `'overlay'` を外した。**余白「なし」なら、選んだ辺に写真の上から重ねる**
  （`StyleDef.caption.overlay`）。入口が1つになり、上・下・左・右のどこにも重ねられる
  - 暗幕は文字の辺から内側へ。上下は従来の奥行き（320lu、16:9 は 230lu）。左右は段（端から 34＋300lu）を
    平らな濃さで覆うよう、奥行き 520lu・立ち上がり 0.35（`scrimAlphaAt` に plateauAt を足した）
  - コントラスト 4.5:1 の検査を4辺に広げた（文字の箱の、辺からいちばん遠い端で測る）
  - 旧い保存の `'overlay'` は「余白なし・下」と読む（見た目は以前のまま）
- 「揃え（左右）」と「寄せ（上下）」を **3×3 の点1つ**に。上辺・下辺に重ねるときは上下が辺で決まるので、その行だけ生きる
- 刻印は文字と同じ部品（置き場所の絵・3×3・スライダー・スイッチ）
- 押しても使えなかった「組み方向」の列は、縦組みができるまで出さない
- 総当たりは 2,610 → 2,808 通り（全面が 4辺 × 寄せ3 に増えた）

**道具（タブ）は6→5**: フレーム（比率・余白・写真・地色・枠線）／文字（置き場所・位置・行数・大きさ・字間）／
書体／刻印／情報。余白・地色・枠線はどれも額の性質なので1つに。

**部品（`ui/controls.tsx`）**: 値の種類ごとに形を使い分ける。縦に回すホイール（Wheel・Segmented・Choice）は廃止。
| 値 | 部品 |
|---|---|
| 段のある量（余白・大きさ・字間） | 段つきスライダー（input[type=range]。両端の言葉と今の値。ダブルクリックで既定へ） |
| 形で分かる選択（比率・置き場所・写真の位置・行数） | 絵のボタン（額＝外枠、写真＝塗り、文字＝太線の3要素で描く） |
| 帯の中の位置 | 3×3 の点 |
| 地色 | 色の丸（選んだ色の名前を見出しの下に） |
| 書体 | 字形カード（「Aa」をその書体で） |
| 載せる／載せない・枠線 | スイッチ |

**見え方の決まり**: 選択中＝白の線＋一段明るい地（`--surface-2`）。押せない＝`--ink-3`（#77736d に上げた。
以前は 2.9:1 で読めなかった）で薄く、押すと理由が1行。道具の列は地を塗らず、選択中だけ白い字＋上の細線
（以前の白い丸は写真の白いフチに次いで明るく、目を引いた）。操作面は5つの道具で同じ高さ（204px）、長い面は中で送る。

**情報**: 8項目を「名前・今の中身・スイッチ」で全部並べる。行を押すと情報シートがその欄から開く。
空の項目は「未入力」「記録なし」。下に「まとめて編集」「情報を戻す」。

**PC**: 右の欄は見出しで畳める（畳んだ見出しに今の値）。見出しは送っても上に残る。
「書き出す」は欄の下にいつも見えている（Ctrl/⌘+S）。見出しの共有の印は PC では出さない。

**検査**: 単体（4辺の重ね・寄せ・コントラスト・旧保存の読み替え）、実ブラウザ 30 件、`shot.mjs`
（新しい道具の名前・比率の形のボタン・PC の欄の部品のはみ出し・送り途中の行を指の検査から外す）。

### 3.24 UI/UX 見直し・残りの11件（2026-09-23）

- **U01 プレビューの文字が読めない** → スマホでダブルタップすると、押した所を中心に 2.5 倍で見られる。
  拡大の間は指で送って見回し、もう一度ダブルタップか「2.5× ✕」で戻る。拡大の間だけ細かく描き直す
  （上限 2240px、`makePreviewTarget` の zoom）。余白なしのときも、1本指のドラッグは切り取り、ダブルタップは拡大。
  最初の3回だけ足元で「ダブルタップで文字を拡大」。注記は指を遮らない（pointer-events: none）
- **U06 書き出しの窓**: 保存できたら題を「保存しました ✓」に。主ボタンは1本、ほかの保存の仕方は文字のリンク。
  「アプリを共有」は控えめな文字に
- **U11 次の写真へ**: 保存できたら「続けて編集する」を「次の写真を選ぶ」に置き換える。選び終えたら窓を閉じ、
  設定はそのまま（複数枚は §8 のフィルムストリップで段階的に）
- **U08 撮影情報が無い写真の帯**: 見出し1行（ファイルの日付入り）と短い出口3つ（日付を使う／手で入力／なしで作る）を横1列。
  高さ 84px（以前の約半分）
- **U16 情報シート**: カメラ・レンズは見出しの下に全幅（写真の値を最後まで見せる）。日付の欄が空なら「写真：2026.09.21」を添える。
  タイトル・作者の見本は「省略可」
- **U17 LINE の中のブラウザ**: 起動時に `openExternalBrowser=1` を付けて一度だけ開き直す（boot.js、繰り返さない印つき）。
  それでも中のブラウザなら、ホームに1行だけ出口を出す
- **U19 背の低い端末**: プレビューの左右の余白を 16px に。高さ 620px 以下では見出し 40px・道具の列 54px・上下の余白 12px
- **U22 写真を動かせること**: 切り取っているときだけ掴める（元比では掴む印を出さない）。余白なしにしたとき起動ごとに1回
  「写真を指で動かして位置を決められます」（PC は「ドラッグで…」、元比なら「比率を変えると位置を動かせます」）。
  PC は矢印キーで 2%（Shift で 10%）、ダブルクリックで真ん中へ
- **U25 PC の注記**はプレビューの右下（設定の欄のそば）に出し、4秒残す
- **U28 動画の音の印**: 押せる印にだけ枠。音のある動画が流れ始めたら1回「スピーカーの印で音を出せます」
- **U29 PC のホーム**: 落とせる場所を破線で囲む。＋の説明に「ドロップ・Ctrl+V でも開けます」

### 3.25 写真と文字の関係を作り直した：触った軸だけが変わる（2026-09-23）

依頼者の指摘（実機）:
- 文字を左右の段にしているとき、写真を左右に寄せると文字が勝手に下へ移った（混乱した）
- 写真を左に寄せてから文字を右の段にすると、写真の位置が変わった。写真と文字のどちらが優先なのか分からない
- 文字を入れたくない写真もある

原因は3つ。(1) 写真の位置の5択は、余りの無い軸では効かない（1:1 に横長の写真なら左右は効かない）。
(2) 効かない組み合わせを避けるため、doc.setStyle と normalize が**相手の軸を書き換えていた**。
(3) 文字は額を基準に置いていて、写真を寄せても文字は額の中央に残った。

**決めたこと（依頼者が選んだ）**
- **触った軸だけが変わる。** 相手の軸を書き換える規則はすべて外した（normalize は旧保存の読み替えだけ）
- **写真の位置は 3×3 の点**（`PhotoPlace` を9通りに）。写真は額の余りの中に収まるので動けるのは横か縦の一方だけ。
  その時に動ける方向の点だけを生かし、ほかは薄く・押すと理由（`layout.freedom` → Scene の meta → 操作面）。
  値は書き換えずに覚えておき、比率や写真を替えて余りが出れば、その位置に効く
- **文字は写真に揃える。** 上下の帯は写真の幅（左揃えなら写真の左端から）。写真より長い文字だけ、揃えの向きに
  額の内側まで広げる。左右の段は写真の高さの範囲で上・中・下（写真より高ければ額の内側に収める）。
  文字の3×3 の縦も、帯に余りが無ければ薄くして理由を出す
- **「文字を入れる」スイッチ**（文字タブの先頭、好みとして保存）。切ると帯ごと消え、写真は余白の中に収まる。
  文字の設定は薄く残り、入れ直せば元どおり。情報タブには「文字は入れない設定です」を1行
- PC の「撮影情報がありません」の帯のボタンは横に伸ばさない
- 総当たりは 6,048 通り（写真9 × 文字4 × 寄せ3 × 余白3 ＋ 全面 4辺 × 寄せ3、× 比率6 × 行数3）

### 3.26 刻印の仕上がりを刻印タブで選ぶ・小さい段・行末の「,」（2026-09-23）

- **刻印タブの先頭で仕上がり（刻む名前）を選ぶ。** 選んだ値は情報の「仕上がり」（手入力）と同じもので、情報にも出る。
  以前は情報シートで仕上がりを入れてからでないと刻印が効かず、どこで何をすればよいか分からなかった。
  候補に無い名前は「その他（手で入れる）…」で情報シートの仕上がりの欄から入れる
- **刻印の大きさを5段に**（極小・より小・小・中・大）。小さい2段は札の高さ 32lu・48lu（基準 16lu の 2倍・3倍）
- **折り返しは項目の切れ目で。行末に区切りを残さない**（`wrapFields`）。以前は区切りごとつないだ文字列を空白で折っていて、
  「yashima takeshi,」のように行末に「,」が残った。1項目が1行に入らないときだけ、その項目の中を空白で折る
- 1行の組みに作者が入っていなかった（情報で作者を載せても1行だと出なかった）のを直した

### 3.27 行の割り振り・余白の極狭・全画面・一覧の色（2026-09-23）

- **項目を何行目に出すかを利用者が決める**（情報タブ）。`LineLayout`＝いつも3組（1行目〜3行目）、組の中は左からの順。
  - 組の数は「行数」と連動する。行数が少ないときは、はみ出した組を最後の行に続ける（`groupsFor`）。
    行数の少ない状態で並べ替えると、見えている組の上で動かし、はみ出していた項目は最後の行に書き戻す
  - 操作: 各項目のつまみ（⠿）をドラッグ（指・マウス）。落とす場所に白い線。面の端では自動で送る。
    キーボードはつまみで上下キー（行の端では隣の行へ）。情報タブの先頭にも「行数」を置いた（文字タブと同じ設定）
  - 既定は以前の固定の組みを再現する（3行: タイトル・作者・日付／カメラ／レンズ・仕上がり・焦点距離・露出）。
    2行は1行目以外を2行目に続けるので、以前2行目から抜けていた焦点距離が入るようになった
  - 行ごとの見え方（3行: 標準／太字／小さく薄く）は行の位置で決まり、組の見出しに添える
  - 好みとして保存（壊れた保存は既定へ。項目が増えたら最後の行の末尾へ足す）。「情報を戻す」で既定へ。取り消せる
- **余白に「極狭」**（倍率 0.22）。なし・極狭・狭い・標準・広い の5段
- **全画面で見る**: 見出しの印（PC は F キー）。画面いっぱいに同じ描き手で描く。✕・背景・Esc・戻るで閉じる（#view）。
  動画は流れたまま
- 選択の一覧（仕上がり・日付の書き方など）を暗い地に明るい字に。PC の Chrome で暗い地に黒い字になって読めなかった

### 3.28 ロゴ切れ・全画面の拡大・選べる点・4行・SNS の比率・欄の幅（2026-09-23）

**直したこと**
- **16:9 で刻印（ロゴ）がキャンバスの外に出た**: 文字を写真の幅に揃えるようになって帯が狭くなり、文字と刻印が横に重なって
  縦に積む高さが要るのに、広い帯（額の幅）で判定していた。実際の帯で見直し、要れば組み直す（compose.ts）。
  比率 × 写真の位置 × 刻印の左右 × 大きさ で外に出ないことを検査に足した
- **全画面が拡大されなかった**: canvas に max-width/max-height しか当てておらず、描いた大きさ（最大 1400px）より
  大きくならなかった。画面いっぱいに object-fit: contain で広げ、細かく描く（上限 2240px）。PC はブラウザも全画面にする
- **選べる点が分かりにくかった**: 3×3 の点を 押せる＝白に近い輪 ／ 選んでいる＝白く塗った四角 ／ 動けない＝小さな点 に分けた
- **スマホで 3×3 の点や比率の形が次の行に重なった**: 操作面が grid で行が 44px に潰れていた。flex で縦に積み、行は中身の高さまで伸びる。
  shot.mjs に「行の中身が行からはみ出さない」検査を足した
- **PC の欄で比率が切れた**: 欄の既定の幅を 440px に、比率は欄が狭ければ折り返す。**欄の幅は左の縁のつまみで変えられる**
  （340px〜窓の半分、ダブルクリックで既定、左右キーでも。この端末に覚える）
- 欄のスクロールバーを細く、地に溶かした

**足したこと**
- **行数は4行まで**。4行は 標準／太字／小さく薄く／小さく薄く。既定の割り振りの4組目は 焦点距離・露出（3行以下では3行目に続く＝以前と同じ）
- **入る行数を制御**: 比率・写真・文字の大きさで入る行数が変わる。行数ごとに組んでみて、上下の帯（重ね）ではキャンバスの高さの 1/3、
  左右の段では 85% を超える行数は減らす（`linesFit`）。行数の選択で入らない行数は薄くし、押すと「この比率・文字の大きさでは○行まで」。
  選んだ行数は覚えておき、入るようになれば戻る。重ねの暗幕は文字の高さに合わせて奥行きを伸ばす（4行・大きい字で端の行が薄い所に出た）
- **文字の大きさに「極小」**（10.5lu）
- **SNS の比率**: 2:3（Threads・X の縦長）と 1.91:1（Instagram の横）を足して8つ。使いどころを読み上げと title に添えた
  （4:5 Instagram の縦／3:4 Instagram のプロフィール・Threads／1:1 Instagram・X／9:16 TikTok・リール・ストーリーズ／16:9 X・YouTube）
- 総当たりは 14,208 通り（8比率 × 4行 × (余白4 × 写真9 × 文字4 × 寄せ3 ＋ 全面 4辺 × 寄せ3)）

### 3.29 役割で分け直した文字と情報・その場で入力・区切り・枠の太さ（2026-09-24）

- **文字と情報を役割で分け直した**（依頼者と相談して決めた）。
  - 文字タブ＝どこにどう見せるか（入れる・置き場所・位置・大きさ・字間）。行数は「◯行・並びと区切りは『情報』で ›」の入口だけ
  - 情報タブ＝何を何行にどう並べるか（行数・区切り・日付の書き方・項目の並び・中身）。行数は情報タブだけに置いた
    （以前は両方にあって、連動していることが分からなかった）
- **情報はその場で直接入力**（依頼者の提案。以前は別の画面＝情報シートを開いていた。情報シートは廃止）。
  タイトル・作者・カメラ・レンズは文字の欄（空ならカメラ・レンズは写真の値を見本の字で見せ、そのまま載る）、日付は日付の欄、
  仕上がりは候補から（刻印タブと同じ部品 FilmSelect。「その他」でその場の欄に打つ）、露出・焦点距離は写真の値だけ。
  刻印の「その他」・撮影情報の無い写真の帯の「手で入力」は、情報タブのその欄に入力位置を置く
- **区切りを選べる**: , ／ · ／ / ／ — ／ | ／ 空白。好みとして保存
- **枠線の太さ**: なし・極細（ヘアライン）・細い 3lu・中 6lu・太い 12lu。**写真の外側に引く**（写真を隠さない。
  外側の余白が足りなければ入る分だけ外へ）。全面では内側に引く。入り切りと太さは1段の取り消し
- 地色の丸の選択の輪が上下で欠け、上の行に被って見えていたのを直した（横に送る列は縦にも切り抜かれる。上下に余白を取った）

### 3.30 スマホの設定欄を1項目ずつに・浮かぶ道具の帯・写真だけのピンチ（2026-09-25）

依頼者の指摘: 「設定の欄が縦に広すぎて、フレームを付けた写真が小さくなりすぎて編集しづらい」
「設定の欄をピンチすると画面ごと拡大されてレイアウトが崩れる。拡大縮小できるのは写真だけに」
「道具のタブはもう少し下に置けるはず。最近の Instagram や iPhone の編集画面のタブの形に」。
実測（iPhone の Safari 相当 390×664）で、写真の欄は 316px（画面の48％）、写真そのものは 177×268px だった。

- **設定欄は項目を1つずつ出す**（依頼者が3案から選んだ。iPhone の写真アプリの編集と同じ考え方）。
  道具ごとに項目の名前を小さなボタンで横に並べ（フレームなら 比率・余白・写真・地色・枠線）、選んだ1つの操作だけを出す。
  - 欄の高さはどの道具・どの項目でも 130px（名前の列 46px＋操作 84px。操作でいちばん背が高い3×3の点が 72px）。
    切り替えてもプレビューが跳ねない。以前は全部の行を積んだ 236px
  - 情報の「項目」（その場で入力・並べ替えの一覧）だけは、送りながら編集するので欄を高くする（画面の34％・最大300px）。
    情報タブを開くと「項目」から。刻印の「その他」・撮影情報なしの帯の「手で入力」も「項目」に着く
  - 道具ごとに、最後に触った項目を覚えておく（戻ってきたとき続きから）
  - 効かない項目は消さずに薄く（押すと理由が出るのは前と同じ）。右に続きがあるときは名前の列の右端を薄く消す
  - PC の欄は今までどおり全部の行を並べる（ui/tools.tsx の ToolPanel が、置かれた場所で並べ方を変える）
- **道具の帯は下に浮かぶ丸い帯に印だけ**（依頼者の指定。Instagram の形）。選んでいる道具は明るい丸で囲み、丸は滑って移る。
  ホーム画面から開いたときは安全域に少し食い込ませ、画面の下端から 22px（以前より約16px下）。
  印だけでは「刻印」「情報」が分かりにくいので、使い始めの8回は押したときに名前を一瞬出し、長押しの間はいつでも出す
- **ピンチは写真の上だけ**（useStageZoom）。2本指で 1〜5倍、拡大中は1本指で見回し、ダブルタップで 1倍 ⇄ 2.5倍。
  動かしている間は transform だけを書き換え、指を離したらその倍率に合う細かさで描き直す（にじまない）。
  余白なしの切り取りドラッグは、拡大していないときの1本指のまま（2本目の指が来たら切り取りをやめて拡大に譲る）。
  全画面表示でも2本指で拡大できる（1回押すと閉じるので、ダブルタップの拡大は使わない。拡大中は押しても閉じない）
- **ページそのものは拡大させない**（スマホ）。touch-action: pan-x pan-y（欄の送りは残す）に加え、
  iPhone の Safari が touch-action だけでは止めないことがあるので gesturestart と2本指の touchmove の既定の動きも止める。
  PC ではブラウザの拡大を残す
- 写真の欄の上下の余白を 24px→12px に詰めた
- 結果（同じ 390×664）: 写真の欄 316px→420px、写真 177×268px→261×396px（面積約2.2倍）。
  iPhone SE の Safari（375×553）は 131×199px→201×305px。ホーム画面から開いた 390×844 は 339×513px

### 3.31 意味の近い項目を組に・真ん中に揃える・左右に払って道具を移る・長押しで全画面・行数は1か所に（2026-09-25）

依頼者の指摘: 「1項目ずつは細切れ過ぎ。意味が同じものは同じ組に」「道具の帯も項目も左に寄っていて、場所を持て余している」
「左右に払って道具を移り、すばやく設定を替えたい」「写真を長押しするとコピーの網掛けが出て操作しづらい。長押しで全画面に」
「行数を選ぶ所が文字にも情報にもある。いちばん良い置き場所に」。

- **項目を組にまとめた**（ui/tools.tsx の Line・Cells）。組の中は「小さな見出し＋部品」の行を縦に（Line）か、横に並べる（Cells）。
  - フレーム: 比率／余白・地色（額の見え方）／写真（位置・枠線）
  - 文字: 配置（入れる・置き場所・位置）／大きさ・字間
  - 刻印: 仕上がり（フィルム・刻印の出し方）／配置（置き場所・位置）／大きさ・枠線
  - 情報: 項目（行数・一覧）／区切り・日付
  - PC の欄は今までどおり1行ずつ（組を行に分ける）
- **真ん中に揃える**。項目の名前の列と組の中身は、入りきるときは画面の真ん中に（はみ出すときは左から送れる。auto の余白を使う）。
  中身の幅は最大 440px。欄の高さは名前の列 46px＋組 100px＝146px（どの組でも同じ。情報の「項目」の一覧だけ高い）。
  文字の配置（切り替え・置き場所・位置）は 375px の iPhone でも1行に入るよう、間を幅に合わせて詰める。
  320px の端末では組ごと横に送れる
- **設定欄を左右に払うと隣の道具へ**（editor/OptionRow.tsx）。左へ払う→右隣（フレーム→文字→書体→刻印→情報）、右へ払う→左隣。
  端では止まる（ぐるりと回らない）。移った道具は払った向きから滑り込む（動きを減らす設定では滑らない）。
  横に56px以上・0.7秒以内・縦の1.6倍以上横に動いたときだけ払いとみなす。
  スライダー・入力欄・並べ替えのつまみの上と、横に送れる列（書体の見本など）の上から始めた指は、その部品に任せる。
  欄の中の送れる箱（項目の名前の列・絵の列・情報の一覧など）は、横に送る中身が無いときは横の指を手放す（data-swipe）。
  手放さないと、ブラウザが横の指を取って払いが届かず、情報の一覧の上ではページが前の画面に「戻る」ことがあった。
  横に送り切った指も「戻る」に抜けないようにした（overscroll-behavior-x）
- **写真を長押しすると全画面**（useStageZoom の onLongPress、0.48秒・8px以内）。拡大中は見回しを優先して開かない。
  端末の選択・コピーのメニュー（網掛け）は出さない（contextmenu を止め、user-select と -webkit-touch-callout を切る）。
  指を離したときの click が開いた全画面に落ちて即座に閉じるので、直後の1回は捨てる
- **行数は情報の「項目」の一覧の先頭だけ**。行数は「何を何行目に出すか」と一体の設定なので、行の組の真上に置く。
  文字タブの行数の入口はやめた（同じ設定が2か所にあるように見えた）
- 結果（390×664）: 写真の欄 404px、写真 251×380px（§3.30 の 261×396px より組の分だけ少し小さい。組に分けて押す回数を減らすのを優先）
